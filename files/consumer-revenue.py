...
ANNEXURE = "1.11"
TABLE_HEADER = "Consumer Category-wise Revenue per Unit"

INPUT_FOLDER = r"C:\Users\ribhu\csep-hermes\input"

# ALL CSV FILES GO INSIDE THIS ONE FOLDER
OUTPUT_FOLDER = (
    r"C:\Users\ribhu\csep-hermes\outputs\Consumer Category-wise Revenue per Unit\23-24"
)
...
NUM_COLS = 6
UNITS = ["Rs/kWh", "Rs/kWh", "Rs/kWh", "Rs/kWh", "Rs/kWh", "Rs/kWh"]
...
skip_patterns = [
    'Consumer Category-wise Revenue per Unit',
    'Rs /kWh',
    'Average Revenue',
    'Domestic Commercial Agricultural Industrial Others per Unit Sold',
    'Annexure',
    'Section 1',
...
def find_annexure_blocks(pdf, annexure_id, header, max_span=5):
    ...
    prior years (e.g. this annexure appears once for 2023-24, again
    for 2022-23, and again for 2021-22 -- all still tagged with the
    same annexure number). This function keeps scanning PAST each
    block it finds instead of stopping at the first one, and reads
    the year label printed in each block's own header text rather
    than assuming it from a filename, since all blocks can live in
    the SAME pdf file.

    Two different year-label formats show up in this report:
    flow-type tables print "2023-24"; point-in-time balance-sheet
    tables print "As on March 31, 2024" instead. Both are handled.
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for year_label, page_indices in blocks:
            ...
            output_path = os.path.join(
                OUTPUT_FOLDER,
                f"Consumer-Category-Revenue-per-Unit-{short_year}.csv"
            )
